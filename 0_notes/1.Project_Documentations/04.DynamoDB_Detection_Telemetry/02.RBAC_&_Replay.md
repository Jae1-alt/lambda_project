...
sequenceDiagram
    autonumber
    participant Client
    participant APIGW as API Gateway (with Cognito Authorizer)
    participant Cognito
    participant Lambda as RBAC Lambda
    participant DDB as DynamoDB

    %% Phase 1: Managed Perimeter Security
    Client->>APIGW: HTTP Request (Authorization: Bearer <IdToken> + x-token-id)
    
    rect rgb(240, 248, 255)
        note over APIGW, Cognito: Perimeter Authentication Phase
        APIGW->>Cognito: Validate JWT (Signature, Expiry, Audience)
        Cognito-->>APIGW: Token Verified (Returns Identity Claims)
    end

    %% Phase 2: Authorization & Anti-Replay
    note over APIGW, Lambda: APIGW injects claims into context.authorizer.claims
    APIGW->>Lambda: Invoke Backend with User Context
    
    critical 1. The Bouncer: RBAC Evaluation
        Lambda->>Lambda: Inspect Cognito Groups from Context
    option User Not in Required Group
        Lambda-->>APIGW: Return JSON {statusCode: 403, error: "Unauthorized Group"}
        APIGW-->>Client: 403 Forbidden
    end
    
    critical 2. The Bartender: Anti-Replay Verification
        Lambda->>DDB: UpdateItem (Set used="true" WHERE used="false" AND token_id=x-token-id)
    option ConditionalCheckFailedException (Token Already Used)
        DDB-->>Lambda: Error: Condition Check Failed
        Lambda-->>APIGW: Return JSON {statusCode: 403, error: "Replay Detected"}
        APIGW-->>Client: 403 Forbidden
    end
    
    %% Phase 3: Core Execution
    DDB-->>Lambda: Write Success (State committed atomically)
    
    note over Lambda: 3. Execute Core Business Logic Safely
    Lambda->>Lambda: Process Application Payload
    
    Lambda-->>APIGW: Return JSON {statusCode: 200, body: {...}}
    APIGW-->>Client: 200 OK
