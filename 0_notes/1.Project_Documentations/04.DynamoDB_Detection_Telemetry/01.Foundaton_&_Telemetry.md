...
```mermaid
graph LR
    subgraph Client [Client Side / Attacker Vector]
        User[User / Local Script]
    end

    subgraph AWS [AWS Cloud Lab Boundary]
        Cognito[Cognito User Pool]
        API[API Gateway]
        RBAC[RBAC Lambda<br/>'Bouncer & Bartender']
        DDB[(DynamoDB<br/>token-tracking)]
        
        subgraph Automation [Custom Cron Automation]
            EB[EventBridge<br/>Scheduler]
            Janitor[Janitor Lambda<br/>Detection]
        end
    end

    %% Phase 1: Token Generation & Direct Injection
    User -- "1. Authenticate" --> Cognito
    User -- "2. Direct Write<br/>PutItem (used=false)" --> DDB

    %% Phase 2: Execution Path
    User -- "3. API Request<br/>(IdToken + x-token-id)" --> API
    API -- "4. Forward Request" --> RBAC
    RBAC -- "5. Validate JWT" --> Cognito
    RBAC -- "6. Check & Update<br/>Token State" --> DDB

    %% Phase 3: Scavenger Loop
    EB -- "7. Trigger<br/>(Every 5 Mins)" --> Janitor
    Janitor -- "8. Scan/Query & Expire<br/>Unused Tokens" --> DDB

    %% Visual Styling for Lab Elements
    classDef labElement fill:#fff2cc,stroke:#d6b656,stroke-width:2px;
    class User,DDB,Janitor labElement;
```
