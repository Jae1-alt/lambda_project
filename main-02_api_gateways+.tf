...
# http API Gateway doesn't natively work with WAF, thus is omitted for WAF based projects
...
# # Integrations ----------------------------------------
# resource "aws_apigatewayv2_integration" "for_lambda" {
#   for_each = var.function_code_config

#   api_id           = aws_apigatewayv2_api.example.id
#   integration_type = "AWS_PROXY"
#   integration_uri  = aws_lambda_function.jae_lambda[each.key].invoke_arn
# }
...
#   target    = "integrations/${aws_apigatewayv2_integration.for_lambda[each.key].id}"
# }
...
#   statement_id  = each.value.statement_id
#   action        = "lambda:InvokeFunction"
#   function_name = aws_lambda_function.jae_lambda[each.key].function_name
#   principal     = "apigateway.amazonaws.com"
# }
