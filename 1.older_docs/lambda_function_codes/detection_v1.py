...
def lambda_handler(event, context):
    print("[INFO] Detector Lambda invoked. Scanning for stale tokens...")

    # ==========================================================
    # 2. CALCULATE THE TIME THRESHOLD
    # ==========================================================
    # Get current time (timezone-aware) and subtract the threshold (e.g., 10 minutes)
    current_time = datetime.now(timezone.utc)
    threshold_time = current_time - timedelta(minutes=10)
    
    # Convert to ISO8601 string. Because our Sort Key is a String, 
    # DynamoDB can perform lexicographical (alphabetical) comparison 
    # on ISO8601 strings perfectly.
    ...
    # ==========================================================
    try:
        # Instead of a massive Scan, we use a highly efficient Query 
        # against the StatusTimeIndex GSI we built in Phase 1!
        ...
        for item in stale_tokens:
            ...
            # Note: We don't need a ConditionExpression here because this is an 
            # automated background janitor process, not a user-facing API request.
            table.update_item(
